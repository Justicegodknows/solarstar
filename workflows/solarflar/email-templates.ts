...
/**
 * Maintenance reminder template modeled on Mehmet Yilmaz's real customer email
 * (plain signature layout, no card/gradient chrome). `bodyHtml` and `greeting`
 * are inserted as-is (HTML) - callers must escape any dynamic values first.
 */
export function renderMaintenanceReminderHtml(params: {
  greeting: string;
  bodyHtml: string;
  ctaHref?: string;
  ctaLabel?: string;
